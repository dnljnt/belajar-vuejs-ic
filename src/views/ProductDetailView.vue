  ...
  <section class="max-w-7xl mx-auto px-6 py-12 md:py-20 text-black">
    ...
    <template v-else-if="product">
      ...
      <nav class="flex items-center space-x-2 text-sm text-gray-500 mb-8 md:mb-12">
        ...
        <span>/</span>
        <span class="hover:text-black transition-colors duration-200 cursor-pointer">{{
          product.category
        }}</span>
        <span>/</span>
      ...
      <div class="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
        ...
        <div class="flex flex-col gap-4">
          <!-- Main Image -->
          <div
            class="aspect-4/5 bg-[#f4f4f5] rounded-3xl overflow-hidden relative flex items-center justify-center p-8 group"
          >
            <img
              :src="product.image"
              :alt="product.name"
          ...
          <div class="grid grid-cols-4 gap-4">
            <div
              v-for="i in 4"
              :key="i"
              class="aspect-square bg-[#f4f4f5] rounded-xl overflow-hidden cursor-pointer hover:ring-1 hover:ring-black transition-all"
            >
              <img
                :src="product.image"
                class="w-full h-full object-cover mix-blend-multiply opacity-70 hover:opacity-100 transition-opacity"
              />
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col">
          <!-- Title & Price -->
          <h1 class="text-3xl md:text-5xl font-bold tracking-tight mb-4">{{ product?.title }}</h1>
          ...
          <div class="mb-10">
            <div class="flex justify-between items-center mb-4">
              ...
              <button class="text-sm text-gray-500 underline hover:text-black transition">
                Size Guide
              </button>
            </div>
          ...
          <div class="flex gap-4 mb-10">
            <button
              ...
              class="flex-1 bg-black text-white py-4 px-8 rounded-full font-semibold text-lg hover:scale-[1.02] hover:bg-zinc-800 transition-all duration-300 shadow-xl shadow-black/10"
            >
              Add to Cart
            </button>
            <button
              class="w-16 h-16 flex items-center justify-center border border-gray-300 rounded-full hover:border-black hover:text-black text-gray-400 transition-all duration-300 group"
            >
              <svg
    ...
    <div v-else class="text-center py-32">
      ...
      <p class="text-gray-500 mb-8">
        Maaf, produk yang Anda cari mungkin telah dihapus atau URL tidak valid.
      </p>
      <RouterLink
        to="/"
        class="bg-black text-white px-8 py-4 rounded-full font-medium hover:bg-gray-800 transition"
      >
        Kembali ke Home
      </RouterLink>
    </div>
